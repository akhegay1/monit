...
func GetMetrics() string {
	...
	for i := 0; i < cnt; i++ {
		chans[i] = make(chan mutils.Vmetric, cnt) //используем буферизованный канал,
	...
	//для кажд типа горутины создается канал
	//массив каналов для трех типов каналов - DB, SSH и http get
	var i int = 0
	var wg sync.WaitGroup
	//cnt - это кол-во метрик и соотв горутин, в строке 63 вычисляется
	wg.Add(cnt)
